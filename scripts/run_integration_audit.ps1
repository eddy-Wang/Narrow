...
function Get-GitRevision {
    ...
    $value = git -C $repoRoot rev-parse --verify --quiet $Revision
...
$manifest = [ordered]@{
    ...
    integration_commit = Get-GitRevision "HEAD"
    source_commits = [ordered]@{
        main = Get-GitRevision "origin/main"
        testing = Get-GitRevision "origin/testing"
        yxh = Get-GitRevision "origin/yxh"
    }
    ...
    runtime = [ordered]@{
        ...
        uv = (& uv --version)
        agent_python = (& uv run --project $agentProject python --version 2>&1 |
            Out-String).Trim()
        simulator_python = (& uv run --project $agentProject --with-editable $simulatorProject `
...
Invoke-AuditStage "simulator_tests" $repoRoot "uv" @(
    "run", "--project", $agentProject, "--with-editable", $simulatorProject,
    "--group", "dev", "pytest", "user-simulator/tests", "-v"
)

Invoke-AuditStage "simulator_report_tests" $repoRoot "uv" @(
    "run", "--project", $agentProject, "--with-editable", $simulatorProject,
    "--group", "dev", "pytest", "user-simulator/tests/test_reporting.py", "-v"
)

...
class BenchmarkUserPolicy:
    ...
    def initial_act(self, state: UserState) -> DialogueAct:
        ...
        return DialogueAct(DialogueActType.INITIAL_REQUEST, reason_code=f"benchmark:{scenario_type}", surface_text=message)
    ...
    def next_act(self, state: UserState, response: AgentResponse, accepted: bool) -> DialogueAct:
        if accepted:
            return DialogueAct(DialogueActType.ACCEPT, reason_code="benchmark:accept", surface_text="That works for me.")
        ...
        if not self.override_applied and state.turn + 1 == override_turn:
            ...
            return DialogueAct(
                ...
                reason_code="benchmark:intent_override",
        ...
        if not matches:
            return DialogueAct(
                DialogueActType.NO_PREFERENCE,
                attribute=attribute,
                reason_code="benchmark:no_additional_preference",
        ...
        return DialogueAct(
            ...
            attribute=attribute,
            values=matches,
            reason_code="benchmark:answer_attribute",

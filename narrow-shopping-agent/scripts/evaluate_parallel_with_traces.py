...
def main() -> int:
    ...
    parser.add_argument("--model", help="Model name; defaults to OPENAI_MODEL from .env")
    ...
    args.model = args.model or os.getenv("OPENAI_MODEL", "gpt-5.5")
    ...
    if not os.getenv("OPENAI_API_KEY", "").strip():
        _fail(
            "OPENAI_API_KEY 为空；请在 narrow-shopping-agent/.env 中配置。",
            "OPENAI_API_KEY is empty; configure it in narrow-shopping-agent/.env.",
        )

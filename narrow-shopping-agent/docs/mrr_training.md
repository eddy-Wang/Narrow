...
From `narrow-shopping-agent`, using the existing Python environment:
...
.venv\Scripts\python.exe scripts\evaluate_parallel_with_traces.py `
  ...
  --ltr-ranker lambdamart --model gpt-5.5 --workers 4 `
...
the reproducible evaluation commands in the root testing guide.
...
For the full backend/simulator checks, set both `OPENAI_MODEL` and
`LLM_MODEL` to `gpt-5.5` in the test process. The simulator gives

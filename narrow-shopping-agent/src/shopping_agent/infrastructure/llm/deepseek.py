...
def active_provider() -> str:
    """Return the configured online provider.

    OpenAI is the public default. The legacy provider remains available as an
    implementation-level compatibility option when selected explicitly, or
    when only its key is present in an older local environment.
    """

    explicit = os.getenv("SHOPPING_LLM_PROVIDER", "").strip().casefold()
    if explicit:
        if explicit not in {"openai", "deepseek"}:
            raise RuntimeError(f"Unsupported SHOPPING_LLM_PROVIDER: {explicit}")
        return explicit
...
def is_configured() -> bool:
    ...
    required_key = "OPENAI_API_KEY" if provider == "openai" else "DEEPSEEK_API_KEY"
    if enabled and not os.getenv(required_key, "").strip():
        raise RuntimeError(
            f"Online mode requires {required_key}; offline fallback is disabled"
        )
    return enabled
...
def request_state_patch(payload: dict[str, Any]) -> tuple[StatePatch, dict[str, int]]:
    ...
    if active_provider() == "openai":
        from shopping_agent.infrastructure.llm.openai_responses import request_state_patch as request
...
def request_dialogue_decision(payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    if active_provider() == "openai":
        from shopping_agent.infrastructure.llm.openai_responses import (
            request_dialogue_decision as request,
        )
...
def repair_dialogue_decision(
    ...
    if active_provider() == "openai":
        from shopping_agent.infrastructure.llm.openai_responses import (
            repair_dialogue_decision as repair,
        )

        return repair(payload, invalid_result, error_message)

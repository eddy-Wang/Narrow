...
LIMITS = {"native": 200, "simulator-benchmark": 200, "simulator-realistic": 100}
...
class Options(BaseModel):
    ...
    model: str = Field(default="gpt-5.5", min_length=1, max_length=120)
    realistic_verbalizer: Literal["template", "openai"] = "template"
...
class Evaluation(Options):
    mode: Literal["native", "simulator-benchmark", "simulator-realistic"]
...
def environment(options: dict) -> dict[str, str]:
    return {"SHOPPING_AGENT_ENABLE_LLM": "true" if options["provider"] == "openai" else "false",
            "SHOPPING_LLM_PROVIDER": "openai", "OPENAI_MODEL": options["model"], "SHOPPING_UI_RERANKER": options["reranker"],
...
class Runtime:
    ...
    def settings_payload(self):
        return self.settings.model_dump() | {"revision": self.revision,
            ...
            "model_presets": ["gpt-5.5", "gpt-5.4", "gpt-5.4-mini"]}
    ...
    def check_options(self, options):
        ...
        needs_key = options.provider == "openai" or options.realistic_verbalizer == "openai"
        if needs_key and not os.getenv("OPENAI_API_KEY", "").strip():
            raise ApiFailure("openai.not_configured", 422)
    ...
    def command(self, job):
        ...
        if job["mode"] == "native":
            command = [sys.executable, str(ROOT/"scripts/evaluate_with_traces.py"), "--catalog", str(self.catalog),
                ...
                "--llm" if cfg["provider"] == "openai" else "--no-llm"]
    ...
    async def api(request: Request):
        ...
        if path == "capabilities":
            return {"catalog": {"available": runtime.catalog.is_file(), "product_count": len(runtime.catalog_products()),
                ...
                "openai_configured": runtime.settings_payload()["openai_configured"],
        ...
        if path == "settings":
            if method == "PUT":
                ...
                if settings.provider == "openai" and not runtime.settings_payload()["openai_configured"]:
                    raise ApiFailure("openai.not_configured", 422)
        ...
        if path == "settings/openai/key" and method == "PUT":
            credential = OpenAICredential.model_validate(data)
            async with runtime.lock:
                if runtime.active():
                    raise ApiFailure("settings.locked_by_active_job", 409)
                ...
                runtime.revision += 1
            return runtime.settings_payload()
        if path == "settings/openai/test" and method == "POST":
            if not runtime.settings_payload()["openai_configured"]:
                raise ApiFailure("openai.not_configured", 422)
            from openai import OpenAI
            async with runtime.lock:
                started = time.perf_counter()
                try:
                    with OpenAI(api_key=os.environ["OPENAI_API_KEY"], timeout=20) as client:
                        await asyncio.to_thread(client.responses.create, model=runtime.settings.model,
                            input="Reply OK", max_output_tokens=8, store=False)
                except Exception:
                    raise ApiFailure("openai.connection_failed", 502)
            return {"code": "openai.connection_ok", "model": runtime.settings.model, "latency_ms": round((time.perf_counter()-started)*1000)}
            ...
            async with runtime.lock:
                ...
                if method == "POST" and parts[-1] == "messages":
                    ...
                    answer = {"role": "assistant", "content": response["message"], "created_at": stamp,
                        ...
                        "model": runtime.settings.model if runtime.settings.provider == "openai" else "local",

...
- **结论先说**：`PreciseReranker` 经过三轮迭代——v1（有回归）→ v2（按根因修复，追回大部分但未反超）→ **v3（用评测数据拟合权重，在同一份留出集上真正反超了原来的 `FallbackReranker`，TechnicalScore 0.642 vs 0.634）**。`orchestration/graph.py` 现在默认已经切换到 `PreciseReranker`（用的就是 v3 拟合权重）。详细方法、验证方式和已知局限见第八节，请务必看完局限部分再决定要不要长期用这版权重。
